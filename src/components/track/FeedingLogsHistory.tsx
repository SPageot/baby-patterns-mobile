...
import { filterFeedingLogs } from '@/lib/feedingLogUtils'
...
export function FeedingLogsHistory({ logs, babies, onEditLog, onDeleteLog, busyLogId }: Props) {
  ...
  const filtered = useMemo(() => {
    const rows = filterFeedingLogs(logs, { babyId, dateYmd }, babies)
    ...
    <View style={styles.wrap}>
      ...
      {!filtered.length ? (
        <Text style={styles.empty}>
          {hasFilters ? 'No feeds match these filters.' : 'No feedings logged yet.'}
        </Text>
      ) : (
