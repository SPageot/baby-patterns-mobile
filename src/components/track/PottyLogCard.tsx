...
export function PottyLogCard({ log, onEditLog, onDeleteLog, busy = false }: Props) {
  const palette = useHomeTheme()
  const theme = getTrackThemeFromPalette('potty', palette)
  const styles = useThemedStyles(createStyles)
  const { date, time } = formatPottyLogStamp(log.atIso)
  const babyName = log.details.babyName?.trim()
  const badges = getPottyContentBadges(log.details)
  const meta = getPottyLogMeta(log.details)
