...
export function BehaviorLogsList({ logs, onEditLog, onDeleteLog, busyLogId }: Props) {
  const palette = useHomeTheme()
  const theme = getTrackThemeFromPalette('behavior', palette)
  const styles = useThemedStyles(createStyles)
  const today = filterLogsForToday(logs, 'behavior')
  ...
  return (
    <View>
      ...
      <BehaviorLogTimeline
        logs={recent}
        emptyMessage="No behaviors logged today—tap the button above to add one."
        onEditLog={onEditLog}
